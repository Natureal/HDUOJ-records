...
////Nickname: Algodevil
////Run ID: 
////Submit time: 2015-02-08 21:45:41
////Compiler: GUN C++
//////////////////////////////////////////////////////
////Problem ID: 4819
////Problem Title: 
...
inline void Build_y(int xp,int x,int p,int l,int r){
    if(l == r){
...
inline void Build_y_2(int xp,int p,int l,int r){
...
inline void Build_x(int p,int l,int r){
    if(l == r){
        Build_y(p,l,1,1,N);
        return;
    }
...
inline void Query_y(int xp,int p,int l,int r){
    if(Y1 <= l && r <= Y2){
...
inline void Query_x(int p,int l,int r){
    if(X1 <= l && r <= X2){
        Query_y(p,1,1,N);
        return;
    }
...
inline void Update_y(int xp,int c,int p,int l,int r){
    if(l == r){
...
inline void Update_y_2(int xp,int p,int l,int r){
    tmax[xp][p] = max(tmax[xp << 1][p],tmax[xp << 1|1][p]);
    tmin[xp][p] = min(tmin[xp << 1][p],tmin[xp << 1|1][p]);
    if(l == r)
        return;
...
inline void Update_x(int c,int p,int l,int r){
    if(l == r){
        Update_y(p,c,1,1,N);
        return;
    }
